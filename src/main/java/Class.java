...
//    Как только мы вызовем session.close() - состояние entity становится detached
    ...
    //Новое занятие:
    //Hibbernate - ORM фреймворк в мире джава, чтобы упростить работу с реляционными БД в контексте ООП
    //Связи, критерии API, HQL:
    //критерии API - дополнительный набор объектно ориентированных методов-запросов, которые помогают в виде метода сделать запрос
    //но это используется в более сложных запросах
    //HQL - Объектно ориентированные запросы в виде методов
    //критерий API - это дополнительный функционал в Hibernate либо JPA, где с помощью критерия мы можем с помощью
    //последовательности методов сформировать запрос
    //Фишка в том, что все операторы, которые нам доступны в SQL (where, update, delete, like и тд. ) все в критериях имеют методы


    //Новое занятие: JPA (Java Persistence API). Связка spring jpa framework with Hibernate 5
    //В Java EE все должно быть стандартизированно. Куча спецификаций-стандартов-реализаций.
    //Hibernate - не спецификация, но используется в Java EE, следовательно эту библиотеку надо стандартизировать.
    //Появилась спецификация JPA - это уже стандарт. По факту интерфейс для работы с ORM технологией в Java.
    //С JPA у нас есть гибкость - JPA как и любая спецификация (как сервлеты и тд) не имеют никакой реализации.
    //Hibernate - имплементатор для JPA
    //Удобство в том, что когда мы используем спецификацию JPA - мы можем легко поменять провайдера
    //(вместо Hibernate что-то другое). Но все равно больше используется чистый Hibernateи
    //Именнованные запросы:
    //Когда мы работаем с SQL, JDBC - мы выделяем наши запросы в константы
    //В Hibernate and JPA - есть именованные запросы
    //Именованный запрос - по факту аннотация, которую мы вешаем на сущность
    //У этого подхода есть большой плюс. Если мы допустим ошибку в запросе, то наше приложение рухнет на старте
    //+ мы записываем эти запросы в 1 месте
    //Что касается кэша и состояния объектов в JPA - все как в гибернейте
    //Вывод:
    //Hibernate можно использовать без JPA. Можно использовать их в связке. Но JPA без Hibernate не работает, так как это простоя спецификация.
    //JPA работает только в связке с каким-либо конкретным имплементатором - Hibernate, Eclipse
}
