...
public class ExController extends ResponseEntityExceptionHandler { //по умолчанию ResponseEntityExceptionHandler не отрабатывает,только если мы его вот так расширяем
    @ExceptionHandler(RuntimeException.class) //если мы наследуемся от ResponseEntityExceptionHandler, то не нужно ставить эту аннотацию на переопределяемых методах
    ...
    @Override //так мы можем не отрабатывать больше в контроллере ошибки валидации. Мы можем перенести их в этот метод и готово. Так как код с обработкой ошибок валидации может повторятся, а так мы убираем одинаковый код
