...
import org.springframework.orm.jpa.JpaTransactionManager; //JPA and Hibernate вещи находятся в одном модуле ORM(модуль Spring) просто в разных директориях
...
//    }  //Это бин из чистого гибернейта и он нам не подходит

    @Bean //Нужен чтобы сгенерировать entityManager - аналог сессии в гибернейте
    public LocalContainerEntityManagerFactoryBean entityManagerFactory() {
        final LocalContainerEntityManagerFactoryBean entityManagerFactoryBean = new LocalContainerEntityManagerFactoryBean();
        entityManagerFactoryBean.setDataSource(dataSource());
        entityManagerFactoryBean.setPackagesToScan("by.tms.entity"); //пакет для сканирования наших классов

        HibernateJpaVendorAdapter vendorAdapter = new HibernateJpaVendorAdapter(); //выбираем какую реализацию ORM будем использовать
        entityManagerFactoryBean.setJpaVendorAdapter(vendorAdapter);
        entityManagerFactoryBean.setJpaProperties(hibernateProperties()); //можем использовать гибернейтовские проперти, так как мы используем гибернейт и настраиваем не JPA, а гибернейт

        return entityManagerFactoryBean;
    }

    @Bean
    //Это интерфейс из JDK.(по факту чисто jdbc) Его имплементируют Connection pull-ы. Это бин не касается гибернейтп
...
//    }  //Это тоже чистый гибернейт
    @Bean //транзакшн менеджер
    ...
    private final Properties hibernateProperties() { //Дополнительные настройки для гибернейта
        ...
        hibernateProperties.setProperty("hibernate.hbm2ddl.auto", "create-drop"); //авто ддл. ДДЛ(Data Definition Language - Язык определения данных) - это когда мы создаем наши структуры (таблицы).
